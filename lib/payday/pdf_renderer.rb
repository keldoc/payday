  ...
  class PdfRenderer
    ...
    def self.invoice_details(invoice, pdf)
      ...
      if defined?(invoice.invoice_date) && invoice.invoice_date
        invoice_date = if invoice.invoice_date.is_a?(Date) || invoice.invoice_date.is_a?(Time)
                         invoice.invoice_date.strftime(Payday::Config.default.date_format)
                       else
      ...
      if defined?(invoice.paid_at) && invoice.paid_at
        paid_date = if invoice.paid_at.is_a?(Date) || invoice.paid_at.is_a?(Time)
      ...
      if defined?(invoice.refunded_at) && invoice.refunded_at
        refunded_date = if invoice.refunded_at.is_a?(Date) || invoice.refunded_at.is_a?(Time)
                          invoice.refunded_at.strftime(Payday::Config.default.date_format)
                        else
                          invoice.refunded_at.to_s
                        end
